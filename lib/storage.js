...
const DEFAULT_FORM = {
  ...
  questions: [
    { id: 'nombre', type: 'text', label: 'Nombres', required: true, options: [], description: 'Escribe tu nombre completo tal como aparece en tu documento de identidad.' },
    { id: 'apellido', type: 'text', label: 'Apellidos', required: true, options: [] },
    { id: 'edad', type: 'number', label: 'Edad', required: true, options: [], description: 'Indica tu edad en años cumplidos.' },
    ...
    },
    {
      ...
      type: 'textarea',
      label: 'Enfermedades, alergias o condiciones médicas importantes',
      required: true,
      options: [],
      description: 'Si no posees ninguna condición médica o alergia, por favor escribe "Ninguna".'
    },
    {
      id: 'motivacion',
      type: 'textarea',
...
const DEFAULT_FORM_INTENCIONES = {
  ...
  layoutMode: 'all-in-one',
  questions: [
    { id: 'nombre', type: 'text', label: 'Nombre', required: false, options: [], description: 'Opcional. Si deseas que tu intención sea leída con tu nombre.' },
    { id: 'intencion', type: 'textarea', label: 'Intención', required: true, options: [], description: 'Escribe tu petición personal, familiar o de agradecimiento para presentarla ante el Santísimo.' }
  ]
};
...
const FORM_LAYOUT_CONFIG_ID = '__form_layout_config__';

const mapDbFormToJs = (dbForm) => {
  if (!dbForm) return null;
  const questions = dbForm.questions || [];
  const layoutConfig = questions.find((q) => q.id === FORM_LAYOUT_CONFIG_ID);
  const filteredQuestions = questions.filter((q) => q.id !== FORM_LAYOUT_CONFIG_ID);
  return {
    id: dbForm.id,
    title: dbForm.title,
    ...
    layoutMode: layoutConfig?.layoutMode || 'all-in-one',
    successMessage: layoutConfig?.successMessage || '',
    successFlyerUrl: layoutConfig?.successFlyerUrl || '',
  };
};

const mapJsFormToDb = (jsForm) => {
  if (!jsForm) return null;
  const dbQuestions = (jsForm.questions || []).filter((q) => q.id !== FORM_LAYOUT_CONFIG_ID);
  dbQuestions.push({
    id: FORM_LAYOUT_CONFIG_ID,
    layoutMode: jsForm.layoutMode || 'all-in-one',
    successMessage: jsForm.successMessage || '',
    successFlyerUrl: jsForm.successFlyerUrl || '',
  });
  return {
    id: jsForm.id,
    title: jsForm.title,
