...
export function getQuantityOptionValue(answers, questionId, optionKey) {
  ...
  return parseQuantityValue(raw[optionKey]);
}
...
export function getParentQuestionsWithOptions(questions, beforeIndex) {
  return questions
    ...
    .filter(
      (q) =>
        ['select', 'checkbox-group', 'quantity-group'].includes(q.type) &&
        q.options &&
        q.options.length > 0
    );
}
...
export function getLimitSourceCandidates(questions, beforeIndex) {
  return questions
    .slice(0, beforeIndex)
    .filter((q) => q.type === 'number' || q.type === 'quantity-group');
}
...
export function enforceQuantityLimits(questions, answers) {
  ...
  questions.forEach((q) => {
    if (q.type !== 'quantity-group' || !q.maxTotalFrom) return;
    ...
    const keys = Object.keys(trimmed).filter((key) => (trimmed[key] || 0) > 0);
    for (let i = keys.length - 1; i >= 0 && total > max; i -= 1) {
      const key = keys[i];
      while (trimmed[key] > 0 && total > max) {
        trimmed[key] -= 1;
        total -= 1;
      }
    }
...
export function canSetQuantityOption(question, option, nextQty, answers, questions) {
  ...
  const tentative = { ...current, [option]: nextQty };
...
export function formatAnswerForDisplay(question, answer) {
  ...
  if (question?.type === 'quantity-group') {
...
export function buildShowWhen(parentQuestion, optionValue) {
  if (parentQuestion.type === 'quantity-group') {
    return {
