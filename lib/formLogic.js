...
function isKitSectionColorsAndSizesComplete(section, sectionAnswer) {
  const normalized = normalizeKitColorSizesAnswer(sectionAnswer);
  return (section.options || []).every((color) => {
    ...
    return getKitColorSizeTotal(normalized, color) === colorQty;
  });
}

function isKitSectionAssignmentComplete(section, sectionAnswer, expectedTotal = null) {
  const normalized = normalizeKitColorSizesAnswer(sectionAnswer);
  const colorTotal = getKitColorTotal(normalized);
  const assignedTotal = getKitSectionFullyAssignedTotal(section, normalized);
  const articleQty = getKitSectionArticleQty(normalized);

  if (section.sharedMaxGroup) {
    if (articleQty === 0) return true;
    if (colorTotal !== articleQty) return false;
    return assignedTotal === articleQty;
  }

  if (expectedTotal !== null && expectedTotal > 0 && assignedTotal !== expectedTotal) {
    return false;
  }

  return isKitSectionColorsAndSizesComplete(section, normalized);
}
...
function isKitPickerKitEntryValid(question, kitKey, entry) {
  ...
  const sectionsComplete = getKitPickerSections(question, kitKey).every((section) => {
    const sectionAnswer = getKitPickerSectionAnswer(entry, section.key);
    const expectedTotal = section.sharedMaxGroup ? null : qty;
    return isKitSectionAssignmentComplete(section, sectionAnswer, expectedTotal);
  });

  const groupsComplete = [...new Set(
    getKitPickerSections(question, kitKey).map((section) => section.sharedMaxGroup).filter(Boolean),
  )].every((groupKey) => getKitPickerSharedGroupTotal(question, kitKey, entry, groupKey) === qty);

  return sectionsComplete && groupsComplete;
}

export function isKitPickerValid(question, answer) {
  if (!kitPickerHasInlineConfig(question)) {
    if (question.required && getQuantityGroupTotal(answer) === 0) return false;
    return true;
  }

  const normalized = normalizeKitPickerAnswer(answer, question);
  const activeEntries = Object.entries(normalized).filter(([, entry]) => {
    const qty = typeof entry === 'object' && entry !== null
      ? parseQuantityValue(entry.qty)
      : parseQuantityValue(entry);
    return qty > 0;
  });
  ...
  return activeEntries.every(([kitKey, entry]) => isKitPickerKitEntryValid(question, kitKey, entry));
}
...
export function normalizeKitColorSizesAnswer(answer) {
  ...
  return {
    articleQty: rawArticleQty > 0 ? rawArticleQty : 0,
    colors,
    sizes: { ...(answer.sizes || {}) },
  };
}
...
export function getKitSectionFullyAssignedTotal(section, sectionAnswer) {
  const normalized = normalizeKitColorSizesAnswer(sectionAnswer);
  if (!sectionHasSizeOptions(section)) {
    return getKitColorTotal(normalized);
  }

  return (section.options || []).reduce((sum, color) => {
    const colorQty = parseQuantityValue(normalized.colors[color]);
    if (colorQty === 0) return sum;
    const sizeTotal = getKitColorSizeTotal(normalized, color);
    return sum + (sizeTotal === colorQty ? colorQty : 0);
  }, 0);
}
...
export function isKitColorSizesValid(question, answer, answers = null, questions = null) {
  const kitAnswer = normalizeKitAnswer(question, answer);
  const sections = getKitSections(question);
  const hasContext = answers != null && questions != null;
  ...
  const sectionsValid = sections.every((section) => {
    ...
    const expectedTotal = hasContext && !section.sharedMaxGroup
      ? getSectionMaxTotal(section, answers, questions)
      : null;
  ...
  if (hasContext) {
    const groupKeys = [...new Set(sections.map((section) => section.sharedMaxGroup).filter(Boolean))];
    groupsValid = groupKeys.every((groupKey) => {
      const groupMax = getSharedGroupMax(question, groupKey, answers, questions);
      if (groupMax === null || groupMax === 0) return true;
      return getSharedGroupTotal(question, kitAnswer, groupKey) === groupMax;
    });
  }

  if (!hasAnySelection) return !question.required;
  return sectionsValid && groupsValid;
}

export function getQuestionValidationResult(question, answers, questions) {
  if (!question) return null;

  const ans = answers[question.id];

  if (question.type === 'kit-picker' && !isKitPickerValid(question, ans)) {
    if (question.required && (!ans || (kitPickerHasInlineConfig(question)
      ? Object.values(normalizeKitPickerAnswer(ans, question)).every((entry) => {
        const qty = typeof entry === 'object' ? parseQuantityValue(entry?.qty) : parseQuantityValue(entry);
        return qty === 0;
      })
      : getQuantityGroupTotal(ans) === 0))) {
      return 'Selecciona al menos un kit para continuar.';
    }
    return 'Completa colores, tallas y artículos de cada kit seleccionado.';
  }

  if (question.type === 'kit-color-sizes' && !isKitColorSizesValid(question, ans, answers, questions)) {
    if (question.required) {
      const kitAnswer = normalizeKitAnswer(question, ans);
      const sections = getKitSections(question);
      const hasAny = sections.some((section) => {
        const sectionAnswer = isMultiSectionKit(question)
          ? normalizeKitColorSizesAnswer(kitAnswer[section.key])
          : normalizeKitColorSizesAnswer(kitAnswer);
        return getKitColorTotal(sectionAnswer) > 0 || getKitSectionArticleQty(sectionAnswer) > 0;
      });
      if (!hasAny) return 'Esta pregunta es obligatoria.';
    }
    return 'Completa colores, tallas y artículos pendientes.';
  }

  if (question.required) {
    if (question.type === 'checkbox-group') {
      if (!ans || ans.length === 0) return 'Selecciona al menos una opción.';
    } else if (question.type === 'quantity-group') {
      if (getQuantityGroupTotal(ans) === 0) return 'Indica una cantidad mayor a 0.';
    } else if (!['kit-picker', 'kit-color-sizes'].includes(question.type)) {
  ...
  if (question.allowFileAttachment && question.fileRequired) {
    const fileUrl = answers[`${question.id}_file`];
    if (!fileUrl || fileUrl.trim() === '') return 'Debes adjuntar un archivo.';
  }

  return null;
}

export function isQuestionAnswerValid(question, answers, questions) {
  return getQuestionValidationResult(question, answers, questions) === null;
}
