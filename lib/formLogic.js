...
function emptyKitPickerSections(question, kitKey) {
  return Object.fromEntries(
    getKitPickerSections(question, kitKey).map((section) => [section.key, { colors: {}, sizes: {} }])
  );
}
...
export function getKitPickerSharedGroupTotal(question, kitKey, entry, groupKey) {
  return getKitPickerSharedGroupSections(question, kitKey, groupKey).reduce((sum, section) => {
    return sum + getKitColorTotal(getKitPickerSectionAnswer(entry, section.key));
  }, 0);
}
...
function trimKitPickerSectionAnswer(sectionAnswer, maxColors) {
  ...
  for (let i = colorKeys.length - 1; i >= 0 && colorTotal > maxColors; i -= 1) {
    const key = colorKeys[i];
    while (parseQuantityValue(colors[key]) > 0 && colorTotal > maxColors) {
...
function trimKitPickerEntryToQty(entry, nextQty, question, kitKey) {
  ...
  getKitPickerSections(question, kitKey).forEach((section) => {
    ...
    sections[section.key] = trimKitPickerSectionAnswer(sectionAnswer, qty);
  });

  getKitPickerSections(question, kitKey)
    .filter((section) => section.sharedMaxGroup)
    .forEach((section) => {
      sections[section.key] = trimKitPickerSectionAnswer(
        getKitPickerSectionAnswer(normalized, section.key),
        qty
      );
    });
...
function isKitPickerKitEntryValid(question, kitKey, entry) {
  ...
  return getKitPickerSections(question, kitKey).every((section) => {
    ...
    if (section.sharedMaxGroup) return true;
